...
export class AuthController {
    ...
    login(@Body() user: LoginDto) {
        if (!user.identifier) {
            throw new BadRequestException('Debe proporcionar un correo electrónico o un nombre de usuario');
        }
    ...
    signUp(@Body() user: SignUpDto) {
        ...
        return this.authService.signUp(user.identifier, user.password, user.confirmPassword);
    }
    @Put(':id')
